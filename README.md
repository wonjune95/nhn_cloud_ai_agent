...
| 출처 표기 | 답변 본문은 근거 문서를 `[n]` 으로 인용한다. 답변 아래에는 접이식 없이 항상 번호가 매겨진 출처 카드가 나온다: `[n] 문서명 서비스칩 › 섹션 원문 ↗`(원문 링크는 http(s) 일 때만). 번호는 프롬프트에 넣은 `[문서 N]` 순서와 같다. |
| 진행 상태 | 답변을 만드는 동안 `검색 중 → 관련도 평가 중 → 답변 작성 중` 순으로 상태 문구가 바뀌고, 그 아래 후보 문서 칩(서비스 · 문서명 › 섹션)을 검색 직후와 리랭킹 직후 두 번 다시 그린다. |
| 스크린샷 크게 보기 | 인라인 스크린샷마다 "크게 보기" 버튼이 있고, 누르면 같은 이미지를 큰 다이얼로그로 띄운다. |
| 다시 생성 | 답변마다 "다시 생성" 버튼이 있다. 같은 질문을 다시 보내 새 답변을 이전 답변 뒤에 덧붙인다(이전 답변은 지우지 않는다). 새 답변도 `questions` 에 별도 행으로 로그되고, 검색 질의는 같은 질문을 스스로의 후속 질문으로 오인해 중복 이어붙지 않도록 처리한다. |
| 첫 화면 예시 | `DNS Plus에서 레코드 세트를 생성하는 방법` · `SMS 발신 번호를 등록하는 절차` · `인스턴스를 생성하는 방법` · `Cloud Monitoring에서 대시보드를 생성하는 방법`. 힌트: "서비스 이름을 함께 쓰면 더 정확합니다 (예: VPC, Object Storage)". |
| 사이드바 | 서비스 선택(`자동` 또는 목록에서 고정 선택), "참고 문서 수" 슬라이더(리랭킹 후 프롬프트에 넣을 문서 수), 세션 범위 대화 목록("새 대화"로 새로 시작, 제목을 눌러 전환, "현재 대화 지우기"로 비움). 최대 20개까지 유지하고(넘으면 오래된 것부터 버림), 제목은 그 대화의 첫 질문 앞 24자다. 브라우저 탭을 닫으면 목록 자체가 사라진다 — `questions.session_id` 로 남는 로그와는 별개다. |
...
관리자 페이지(`/admin`)는 기간(7일/30일/전체)을 고를 수 있고, 전체 지표·일별 추이
차트(질문 수·응답 중앙값·👎)·서비스별 표, 질문 검색창(`ILIKE`, 질문에 포함된 단어로
찾음), 최근 👎/미확인/응답 30초 초과 질문 목록, 인덱스 상태를 보여준다. 👎 받은 질문은
행을 펼치면 전체 답변과 근거 문서 목록까지 볼 수 있다.
...
3차 (2C `2c-762820a`, 인용 지시 `[n]` 추가 후, NIM 재시도 0회): 적중 25/25, 메뉴 경로 20/20, 스크린샷 12/20 (60%), 문서 밖 4/5,
지연 중앙값 10.6초 / 최대 38.2초, 예외 0. 인용 지시가 메뉴 경로·적중을 해치지 않았고, 스크린샷은 2차 대비 2문항 줄었다 —
실행 간 편차(1차→2차에서도 문항이 뒤집힘)와 `[n]`·`{{img:N}}` 두 표기가 단계 끝에서 경쟁할 가능성 둘 다 열려 있어,
👎 답변 원문에서 `[n]` 이 그림 자리에 쓰였는지 관리자 페이지로 확인하는 것을 후속으로 둔다.
