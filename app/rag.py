...
CONSOLE_SYSTEM_PROMPT = _SYSTEM_COMMON + (
    "답변 형식: "
    "첫 줄은 언제나 콘솔 메뉴 경로다. 첫 줄 앞에 이모지나 기호를 붙이지 말고 '콘솔 > ' 로 바로 시작해라. 답변의 근거가 된 문서 블록 머리말의 '서비스: 카테고리/서비스' 값을 "
    ...
    "그 단계의 맨 끝, 인용 번호 [n] 까지 쓴 뒤에 {{img:N}} 를 붙여라 (예: '…클릭합니다. [1] {{img:2}}') (예: '3. 서브넷 생성을 클릭합니다. {{img:2}}'). "
