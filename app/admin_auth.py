...
def gate() -> bool:
    ...
    typed = st.text_input("관리자 토큰", type="password", key="admin_token_input")
    if st.button("확인", key="admin_token_submit"):
        if typed and hmac.compare_digest(typed, required_token()):
