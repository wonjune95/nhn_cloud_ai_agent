...
# 2단계-B-1 배포 검증에서 나온 항목 (2026-09-18, 이미지 2b1-6b60389)

## 2B-2 평가·튜닝에 반영
- **스크린샷은 원문에 있을 때만 나온다** — 콘솔 사용 가이드 87개 중 56개만 이미지 보유, 전체 청크 25,757개 중 1,826개(7%)만 이미지. VPC·Floating IP·Object Storage 콘솔 가이드는 이미지 0장이라 예시 질문 4개 중 3개는 스크린샷이 나올 수 없다. DNS Plus·SMS·Instance 등 이미지 있는 문서에서는 마커·파일 모두 정상(검증 완료). 예시 질문을 이미지 있는 서비스로 바꿀지 결정 필요(스펙 4-1 문구 변경).
- **검색 품질**: "인스턴스를 생성하는 방법" 상위 5개 중 3개가 DB 인스턴스·RAG 생성 등 엉뚱한 청크, 정답 청크는 193자 도입부만 → 답변이 4줄로 짧음. 서비스 부스트(×1.3)가 doc_title/section 일치를 못 잡는다. 평가 30문항에서 doc_title 가중·섹션 인접 청크 병합 검토.
- **추론 ON/OFF 비교**(답변 단계): OFF 는 0.5~7초(ON 11~16초)로 빠르지만 문서에 없는 메뉴 경로를 추정해 쓰는 경향("콘솔 > Network > VPC > Subnet" — 원문에 경로 문장 없음). ON 유지. 단, ON 에서 드물게 영어 사고 과정이 본문으로 새어 나옴("We need to answer…", 1/7 관측) — `chat_stream` 에서 첫 토큰이 영어 추론 패턴이면 재요청하는 가드 검토.
- `documents.ingested_at` 은 ADD COLUMN DEFAULT now() 라 기존 행이 마이그레이션 시각으로 채워짐 → 관리자 "마지막 적재" 는 다음 재적재부터 정확. ingest 가 행을 넣을 때 명시적으로 now() 를 쓰는지 확인.
- `/admin` 은 무인증 공개(스펙 수용). 질문·답변 원문이 보이므로 인증 단계에서 이 경로부터 막는다. 최종 리뷰 권고: Traefik basicAuth 미들웨어를 `/admin` 경로에만 먼저 붙이는 것도 가능.
- 테스트 위생: 챗 AppTest 픽스처가 `schema_ready.ensure_schema` 캐시를 clear 하지 않음(현재 무해).
